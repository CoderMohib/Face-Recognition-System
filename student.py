...
class student:
    def __init__(self,root):
      ...
      #  gender_entry.grid(row=2,column=1,padx=5,sticky=W,pady=5)
        ...
        self.student_table=ttk.Treeview(table_frame,columns=('Department','Course','Year','Semester','ID','Name','Division','Roll-No','Gender','DOB','Email','Phone','Address','Teacher','Photos'),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
           ...
           messagebox.showerror("ERROR","ALL FIELDS REQUIRED",parent=self.root)
            ...
            try:
                ...
                if Update>0:
                     conn=pymysql.connect(host="127.0.0.1",user="root",password="Mohibali123@",database="face_recognizer",port=3305)
                     my_cursor=conn.cursor()
                     my_cursor.execute("update student set DEP=%s, Course=%s, Year=%s, Semester=%s, Name=%s, Divison=%s, Roll=%s, Gender=%s, DOB=%s, Email=%s, Phone=%s, Address=%s, Teacher=%s, PhotoSample=%s where StudentID=%s",(
                                                       self.var_dep.get(),
                                                        self.var_course.get(),
                                                        self.var_year.get(),
                                                        self.var_semester.get(),
                    ...
                    #variable me store kia file path
                     ...
                     while True: #infinite loop
                         ...
                         if face_cropped(my_frame) is not None:
                             ...
                             data_folder_path = r"D:\4 seme\TPL\FACE-RECOGNITION-ATTENDENCE-SYSTEM-main\data"
